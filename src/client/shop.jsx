...
export class Shop extends React.Component {
    ...
    buyLootBox = async () => {
        ...
        if (response.status !== 201) {
            this.setState({error: "Failed getting data: " + response.status, successMessage: null});
            return;
        }
        this.setState({successMessage: "Congratulation loot box bought"});
        ...
        return (
            <div>
                <div>
                    ...
                    <p id={"successParagraph"} className={"alert alert-info"}>{this.state.successMessage}</p>}
                </div>
                <div className={"lootBoxShop"}>
                    <button disabled={(!(this.props.user.geons >= 200))} id={"buyButton"} onClick={this.buyLootBox}
                            className={"btn btn-outline-success buyButton"}>Buy loot box
                    </button>
                    <img src={"./logo.png"}/>
